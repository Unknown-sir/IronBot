...
def _remote_panel_login(panel):
    ...
    form = urllib.parse.urlencode({
        'username': _ib194_row_get(panel, 'username', '') or '',
        'password': _ib194_row_get(panel, 'password', '') or '',
    }).encode()
    req = urllib.request.Request(url, data=form, headers={'Content-Type':'application/x-www-form-urlencoded', 'User-Agent':'IronBot/19.0.5'}, method='POST')
    try:
        with _urlopen_with_local_ssl(req, timeout=18) as resp:
            raw = resp.read().decode('utf-8', 'replace')
            cookies = resp.headers.get_all('Set-Cookie') or []
...
# ==============================
# IronBot v19.0.11 - IronPanel API subscription-only delivery
# ==============================
...
def _ib1911_sub_only_text(result, trial=False):
    ...
    text += (
        ...
        "<b>راهنما:</b>\n"
        "این سرویس از طریق IronPanel ساخته شده و همه کانفیگ‌های فعال داخل همین لینک ساب قرار دارد.\n"
        "لینک بالا را باز کنید، کانفیگ موردنظر را از صفحه ساب کپی کنید و داخل برنامه موردنظر خود وارد کنید.\n"
        "اگر برنامه شما از Subscription پشتیبانی می‌کند، همین لینک ساب را مستقیم در بخش Subscription/Import وارد کنید."
    )
    return text
...
def send_config_to_user(user_chat, result):
    if _ib1911_result_is_ironpanel(result):
        ...
        if link:
            text = _ib1911_sub_only_text(result, trial=_ib1911_order_is_trial_by_result(result))
            errors = []
            r = send_message(user_chat, text, disable_web_page_preview=False)
            if not r.get('ok'):
                errors.append('sendMessage: ' + _telegram_error_with_hint(r))
            return errors
...
WATCHER2_VERSION = 'v19.0.11-ironpanel-sub-only-delivery'
