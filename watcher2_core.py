...
LICENSE_V15_DEFAULTS = {
    ...
    "LICENSE_SERVER_URL": "http://95.38.195.21:36514",
    "LICENSE_KEY": "",
...
def license_base_url():
    return str(CFG.get("LICENSE_SERVER_URL", "http://95.38.195.21:36514") or "").rstrip("/")


def license_curl_base_cmd(timeout="35"):
    # License API should normally be checked directly, not through the Telegram SOCKS proxy.
    # Also bypass any global proxy environment that may exist on the server.
    cmd = ["curl", "-sS", "--connect-timeout", "8", "--max-time", str(timeout)]
    if to_bool(CFG.get("LICENSE_USE_PROXY", "false")):
        proxy = normalize_proxy(CFG.get("PROXY_URL", ""))
        if proxy:
            cmd += ["--proxy", proxy]
    else:
...
def _license_target_for_tcp_test():
    # If license checks are explicitly configured to go through a proxy, test the proxy endpoint.
    # Otherwise test the license server directly.
    if to_bool(CFG.get("LICENSE_USE_PROXY", "false")):
        proxy = normalize_proxy(CFG.get("PROXY_URL", ""))
        if proxy:
            host, port, scheme = _host_port_from_url(proxy, "socks5h")
            return host, port, f"proxy {scheme}://{host}:{port}"
    host, port, scheme = _host_port_from_url(license_base_url(), "http")
    return host, port, f"license server {scheme}://{host}:{port}"
...
def license_health_check_text():
    lines = []
    lines.append("Watcher2 license network diagnose")
    lines.append(f"LICENSE_SERVER_URL={license_base_url()}")
    lines.append(f"LICENSE_USE_PROXY={CFG.get('LICENSE_USE_PROXY', 'false')}")
    if to_bool(CFG.get("LICENSE_USE_PROXY", "false")):
        lines.append(f"PROXY_URL={mask_secret(CFG.get('PROXY_URL', ''))}")
    ok, msg = license_tcp_preflight(timeout=8)
    lines.append(("[OK] " if ok else "[FAIL] ") + msg)
    base = license_base_url()
    for suffix in ("/health", "/public_key.pem"):
        url = base + suffix
        cmd = license_curl_base_cmd("15") + ["-D", "-", "-o", "-", url]
        try:
            p = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
            out = (p.stdout or "")[:500].replace("\r", "")
            err = (p.stderr or "")[:300].replace("\r", "")
            lines.append(f"curl {suffix}: exit={p.returncode}")
            if out:
                lines.append(out)
            if err:
                lines.append("stderr: " + err)
        except Exception as e:
            lines.append(f"curl {suffix}: exception={e}")
    ok, res = license_api_check(skip_preflight=True)
    lines.append("license_api_check=" + json.dumps({"ok": ok, "result": res}, ensure_ascii=False))
    lines.append("راهنما: تست‌های لایسنس به صورت پیش‌فرض از پروکسی تلگرام عبور نمی‌کنند و با --noproxy انجام می‌شوند. اگر TCP یا /health تایم‌اوت شد، پنل لایسنس اجرا نیست، پورت 36514 در فایروال/NAT باز نیست، یا آدرس LICENSE_SERVER_URL اشتباه است. اگر پنل و ربات روی همان سرور هستند، LICENSE_SERVER_URL را روی http://127.0.0.1:36514 بگذارید.")
    return "\n".join(lines)
...
def license_api_check(skip_preflight=False):
    ...
    if not skip_preflight:
        pre_ok, pre_msg = license_tcp_preflight(timeout=8)
        if not pre_ok:
            return False, {"status": "network_error", "message": pre_msg, "hint": "پنل لایسنس را روی سرور چک کنید: systemctl status watcher2-license ؛ سپس curl --noproxy "*" http://127.0.0.1:36514/health و curl --noproxy "*" http://95.38.195.21:36514/health را تست کنید. اگر پنل و ربات روی یک سرور هستند، LICENSE_SERVER_URL را روی http://127.0.0.1:36514 بگذارید."}
    body = {
        "license_key": key,
        "installation_id": get_installation_id(),
        "server_id": get_server_id(),
        "product": CFG.get("LICENSE_PRODUCT", "watcher2-sales"),
        "version": WATCHER2_VERSION,
    }
    url = license_base_url() + "/api/v1/verify"
    cmd = license_curl_base_cmd("35") + ["-X", "POST", "-H", "Content-Type: application/json", "--data-binary", json.dumps(body, ensure_ascii=False), url]
    try:
        p = subprocess.run(cmd, capture_output=True, text=True, timeout=45)
        if p.returncode != 0:
            return False, {"status": "network_error", "message": p.stderr.strip() or f"curl exited {p.returncode}"}
        try:
            res = json.loads(p.stdout)
        except Exception:
            return False, {"status": "bad_response", "message": "invalid JSON from license server", "raw": p.stdout[:300]}
        payload = res.get("payload") or {}
        sig = res.get("signature") or ""
        sig_ok, sig_msg = verify_license_signature(payload, sig)
        if not sig_ok:
            return False, {"status": "bad_signature", "message": sig_msg, "payload": payload}
        if payload.get("ok") and payload.get("status") == "active":
            CFG.set("LICENSE_LAST_OK", now_str())
            CFG.set("LICENSE_LAST_STATUS", "active")
            CFG.set("LICENSE_LAST_MESSAGE", payload.get("message", "license active"))
            # Honor server interval if present.
            if payload.get("check_interval"):
                try:
                    CFG.set("LICENSE_CHECK_INTERVAL", str(int(payload.get("check_interval"))))
                except Exception:
                    pass
            return True, payload
        CFG.set("LICENSE_LAST_STATUS", str(payload.get("status", "invalid")))
        CFG.set("LICENSE_LAST_MESSAGE", str(payload.get("message", "license invalid")))
...
def handle_admin_command(chat_id, text):
    ...
    if cmd == "/setlicenseserver":
        if not arg:
            send_message(chat_id, "فرمت: <code>/setlicenseserver http://95.38.195.21:36514</code>"); return True
        CFG.set("LICENSE_SERVER_URL", arg.rstrip("/"))
        fetch_license_public_key(force=True)
        send_message(chat_id, "✅ آدرس سرور لایسنس ذخیره شد.\n" + license_status_text())
        return True
...
def _create_invoice_after_optional_name(chat_id, msg_from, gb, order_type=CONFIG_ORDER_TYPE, target_order_id=None, amount=None, plan_id=None, inbound_id=None, requested_name="", duration_days=None):
    V1812_prev_create_invoice_after_optional_name(chat_id, msg_from, gb, order_type=order_type, target_order_id=target_order_id, amount=amount, plan_id=plan_id, inbound_id=inbound_id, requested_name=requested_name, duration_days=duration_days)
    if plan_id:
        try:
            p = plan_by_id(plan_id)
...
# ==============================
# watcher2 v18.12.1 buy group tuple compatibility fix
# ==============================
...
WATCHER2_VERSION = "v18.12.1-buy-group-fix"
