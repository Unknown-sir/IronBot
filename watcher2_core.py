...
# ==============================
# IronBot v19.0.4 - panel edit + login URL test + removable local panel
# ==============================
...
def _remote_panel_login(panel):
    ...
    req = urllib.request.Request(url, data=form, headers={'Content-Type':'application/x-www-form-urlencoded', 'User-Agent':'IronBot/19.0.4'}, method='POST')
    try:
        with urllib.request.urlopen(req, timeout=18) as resp:
            raw = resp.read().decode('utf-8', 'replace')
            cookies = resp.headers.get_all('Set-Cookie') or []
...
WATCHER2_VERSION = "v19.0.4-panel-edit-login-url-local-delete"
