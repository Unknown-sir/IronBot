...
def test_telegram():
    ...
    if res.get("ok"):
        for admin in CFG.admin_ids():
            r = send_message(admin, "✅ تست Iron Bot موفق بود.")
...
def license_health_check_text():
    lines = []
    lines.append("Iron Bot license network diagnose")
...
def admin_panel_text():
    base = V187_prev_admin_panel_text() if V187_prev_admin_panel_text else '<b>پنل مدیریت Iron Bot</b>'
    cur = html.escape(CFG.get('CURRENCY_LABEL', 'تومان'))
...
WATCHER2_VERSION = "v18.11.4-all-delivery-retry-1s"
