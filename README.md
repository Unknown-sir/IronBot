...
A 2-user plan now creates exactly **one config/subscription** that can be shared by up to two users/distinct IPs. A 3-user plan still creates one service and sets that service capacity to 3.

- x-ui / 3x-ui: native `limitIp` is set on the same client.
...
- Plan groups, per-plan price, per-plan duration, per-plan panel selection, and 1..100 user/IP capacity for the same single config
- `0` duration means unlimited time; sales-plan capacity is 1..100 users/IPs per single config
