...
[Unit]
Description=Iron Bot - x-ui / 3x-ui Telegram sales bot
...
basic_install_questions(){
    ...
    set_config LICENSE_SERVER_URL "http://95.38.195.21:36514"
    load_config
    ...
    load_config
    if [ -z "${XUI_INBOUND_ID:-}" ]; then
        read -r -p "x-ui inbound ID for sales configs: " v; [ -n "$v" ] && set_config XUI_INBOUND_ID "$v"
    fi
    load_config
...
test_database(){
    load_config
    if [ ! -f "${DB_PATH:-}" ]; then
        echo -e "${YELLOW}[!] DB not found: ${DB_PATH:-}${RESET}"
        return 1
    fi
    python3 - <<PY
import sqlite3, sys
...
install_or_update(){
    ...
    if ! test_license; then
        ...
        echo -e "${RED}[!] Fix LICENSE_KEY / LICENSE_SERVER_URL or the license panel service before starting.${RESET}"
        return 1
    fi
...
configure_menu(){
    while true; do
        load_config
        clear
        echo -e "${CYAN}Configure Iron Bot${RESET}"
...
license_menu(){
    while true; do
        ...
        echo "[1] Show fixed license server URL (${LICENSE_SERVER_URL:-http://95.38.195.21:36514})"
        echo "[2] Set license key              ($(mask_value "${LICENSE_KEY:-}"))"
        echo "[3] Set product                  (${LICENSE_PRODUCT:-watcher2-sales})"
        echo "[4] Toggle license via proxy     (${LICENSE_USE_PROXY:-false})"
        echo "[5] Test license now"
        echo "[6] Back"
        read -r -p "Option: " lc
        case "$lc" in
            1) set_config LICENSE_SERVER_URL "http://95.38.195.21:36514"; echo "License server URL is fixed: http://95.38.195.21:36514"; pause ;;
            ...
            4) [ "${LICENSE_USE_PROXY:-false}" = "true" ] && set_config LICENSE_USE_PROXY false || set_config LICENSE_USE_PROXY true ;;
            5) diagnose_license; pause ;;
            6) return ;;
        esac
    done
}
...
main_menu(){
    ...
    while true; do
        clear
        echo -e "${CYAN}Iron Bot - x-ui / 3x-ui Telegram sales bot${RESET}"
