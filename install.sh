...
test_database(){
    load_config
    if [ -z "${DB_PATH:-}" ]; then
        echo -e "${GREEN}[+] Local x-ui DB is not required in remote-only mode.${RESET}"
        return 0
    fi
    if [ ! -f "${DB_PATH:-}" ]; then
        echo -e "${YELLOW}[!] Optional local DB not found: ${DB_PATH:-}${RESET}"
        return 0
    fi
    python3 - <<PY
import sqlite3, sys
...
try:
    ...
    con.close()
    print("[+] Optional local database readable, but delivery still uses added remote panels only.")
except Exception as e:
    print("[!] Optional DB test failed:", e)
PY
    return 0
}
...
configure_menu(){
    while true; do
        ...
        echo "[12] Optional local DB path       (${DB_PATH:-remote-only})"
        ...
        case "$c" in
            ...
            12) ask_value DB_PATH "Optional local x-ui DB path (not used for delivery)" "${DB_PATH:-}" ;;
