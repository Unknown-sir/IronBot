## v19.0.2 - Remote-only panel routing

- Disabled automatic/local x-ui database delivery fallback.
- Panel list now shows only explicitly added IronPanel API and x-ui/3x-ui API targets.
- Plans/orders without a valid external panel are routed to the first enabled external panel only when safe; otherwise delivery is blocked with a clear admin message.
- Trial configs also use the configured/first external panel and never local DB.
- Local DB install setting is now optional and kept only for compatibility.
