# Iron Bot

**Iron Bot** ربات فروش و مدیریت کانفیگ برای پنل‌های **x-ui / 3x-ui** است. این ربات امکان فروش کانفیگ، مدیریت کیف پول، تعریف پلن، تعریف مشتری ویژه، ساخت کانفیگ تست، اتصال به چند پنل و تحویل خودکار کانفیگ از طریق تلگرام را فراهم می‌کند.

مالک گیت‌هاب: [Unknown-sir](https://github.com/Unknown-sir)  
نام ریپازیتوری: `IronBot`

> مسیرهای داخلی نصب برای سازگاری با نسخه‌های قبلی watcher2 حفظ شده‌اند تا آپدیت روی سرورهای قبلی بدون از دست رفتن اطلاعات انجام شود.

---

## امکانات اصلی

- منوی مشتری و پنل مدیریت تلگرام
- پشتیبانی از چند مدیر ربات
- جلوگیری از تأیید دوباره فاکتور یا افزایش شارژ توسط مدیر دیگر
- کیف پول، پرداخت از کیف پول و افزایش/کسر موجودی توسط مدیر
- مشتری عادی و مشتری ویژه
- درخواست نمایندگی/ویژه شدن توسط کاربر و تأیید یا رد توسط مدیر
- گروه‌بندی پلن‌ها
- قیمت مستقل برای هر پلن
- مدت مستقل برای هر پلن؛ مقدار `0` یعنی بی‌نهایت
- انتخاب پنل تحویل هنگام تعریف هر پلن
- اتصال به چند پنل x-ui / 3x-ui
- فعال/غیرفعال کردن ارسال QR برای هر پنل به صورت جداگانه
- سیستم کانفیگ تست با محدودیت قابل تنظیم
- ساخت عمده رایگان کانفیگ برای مدیر
- نام‌گذاری امن کانفیگ‌ها و جلوگیری از خطای نام تکراری
- ارسال لینک سابسکریپشن فقط وقتی برای همان پنل ثبت شده باشد
- retry تحویل کانفیگ با فاصله ۱ ثانیه
- متن‌های پیش‌فرض برای خوش‌آمد، قوانین، راهنمای اتصال و پشتیبانی
- پشتیبانی اختیاری از سیستم لایسنس

---

## پیش‌نیازها

- سرور لینوکسی با systemd
- Python 3
- ابزارهای `curl`، `sqlite3`، `qrencode` و `openssl`
- پنل x-ui یا 3x-ui محلی یا ریموت
- توکن ربات تلگرام از BotFather
- Chat ID مدیر

اسکریپت نصب در صورت امکان پکیج‌های لازم را روی Debian/Ubuntu و توزیع‌های RedHat-based نصب می‌کند.

---

## نصب سریع از GitHub

```bash
sudo apt update
sudo apt install -y git

git clone https://github.com/Unknown-sir/IronBot.git
cd IronBot
sudo bash install.sh
```

بعد داخل تلگرام به ربات پیام بدهید:

```text
/admin
```

---

## آپدیت از فایل zip

```bash
cd /tmp
unzip IronBot.zip
cd IronBot
sudo cp watcher2_core.py /opt/watcher2/watcher2_core.py
sudo chmod +x /opt/watcher2/watcher2_core.py
sudo python3 -m py_compile /opt/watcher2/watcher2_core.py
sudo systemctl restart xui-watcher2
sudo journalctl -u xui-watcher2 -n 120 --no-pager
```

---

## مسیرهای مهم نصب

```text
/opt/watcher2/watcher2_core.py
/etc/watcher2/config.env
/var/lib/watcher2/watcher2.sqlite
/var/lib/watcher2/qrcodes
/var/log/watcher2
```

نام سرویس systemd:

```text
xui-watcher2
```

---

## تنظیمات

فایل تنظیمات اصلی:

```bash
/etc/watcher2/config.env
```

نمونه تنظیمات داخل فایل زیر قرار دارد:

```text
.env.example
```

بیشتر تنظیمات از داخل پنل مدیریت ربات قابل تغییر هستند.

---

## اتصال چند پنل

از پنل مدیریت وارد شوید:

```text
/admin → 🖥 مدیریت پنل‌ها
```

برای هر پنل می‌توانید این موارد را وارد کنید:

- نام پنل
- آدرس پنل همراه پورت، مثل `http://1.2.3.4:2053`
- Web Base Path اگر پنل مسیر مخفی دارد، مثل `/secret`
- نام کاربری پنل
- رمز عبور پنل
- هاست عمومی برای ساخت لینک کانفیگ
- لینک سابسکریپشن در صورت نیاز
- وضعیت ارسال QR همان پنل

هنگام تعریف پلن فروش، مدیر انتخاب می‌کند که کانفیگ آن پلن از کدام پنل ساخته و تحویل داده شود.

---

## کانفیگ تست

از پنل مدیریت:

```text
/admin → 🧪 کانفیگ تست
```

قابل تنظیم است:

- هر کاربر چند بار بتواند کانفیگ تست بگیرد
- ریست محدودیت تست برای همه کاربران
- ریست محدودیت تست برای یک کاربر خاص با Chat ID
- پنل تحویل تست
- inbound تست
- حجم تست
- مدت تست؛ مقدار `0` یعنی بی‌نهایت

در منوی کاربر دکمه زیر نمایش داده می‌شود:

```text
🧪 دریافت تست
```

---

## انتشار روی GitHub

بعد از استخراج این بسته:

```bash
cd IronBot
git init
git add .
git commit -m "Initial Iron Bot release"
git branch -M main
git remote add origin https://github.com/Unknown-sir/IronBot.git
git push -u origin main
```

یا از اسکریپت آماده استفاده کنید:

```bash
bash scripts/publish_to_github.sh
```

---

## نکات امنیتی

- فایل واقعی `config.env` را روی GitHub قرار ندهید.
- دیتابیس، کلید خصوصی، کلید لایسنس، توکن ربات و فایل‌های `.pem` را commit نکنید.
- `.gitignore` برای جلوگیری از اضافه شدن فایل‌های حساس آماده شده است.
