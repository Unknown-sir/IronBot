...
## Remote-only panel routing / عدم استفاده از پنل محلی

From v19.0.2, IronBot does not use the local x-ui database as the default delivery panel. Add at least one target panel from Telegram admin panel: `Admin → Panels → Add Panel`. Each plan must deliver through one of the added IronPanel API or x-ui/3x-ui API targets. If an old installation has legacy plans pointing to the local panel and only one external panel is active, the bot migrates those plans/orders to that external panel automatically. If multiple external panels exist, edit each plan and choose the correct target panel.

از نسخه v19.0.2 ربات دیگر از دیتابیس محلی x-ui به عنوان پنل پیش‌فرض تحویل استفاده نمی‌کند. باید از داخل تلگرام در بخش `مدیریت پنل‌ها` حداقل یک IronPanel یا x-ui/3x-ui اضافه شود و هر پلن فروش به یکی از همین پنل‌های اضافه‌شده وصل شود.
