...
## پلن یک‌کانفیگه با ظرفیت چند کاربر

در این نسخه «پلن ۲ کاربره» یعنی **یک کانفیگ / یک لینک سابسکریپشن** که حداکثر ۲ کاربر یا IP می‌توانند از همان سرویس استفاده کنند. پلن ۳ کاربره هم فقط یک کانفیگ می‌سازد و ظرفیت همان کانفیگ روی ۳ قرار می‌گیرد.

- x-ui / 3x-ui: مقدار native `limitIp` همان Client برابر ظرفیت پلن می‌شود.
- IronPanel: فقط یک User ساخته می‌شود، `connection_limit` همان User تنظیم می‌شود و مانیتور IP ربات نیز همان ظرفیت را با API Token کنترل می‌کند.
- هر سفارش فقط یک Order/Service اصلی دارد؛ Child Order جدید ساخته نمی‌شود.
- Retry یا تأیید دوباره هیچ کانفیگ تکمیلی ایجاد نمی‌کند.
- فیلد قدیمی IP Limit در ویرایش پلن با «ظرفیت یک کانفیگ» یکپارچه شده است.
...
- گروه‌بندی پلن‌ها، قیمت مستقل، مدت مستقل، پنل تحویل مستقل و ظرفیت ۱ تا ۱۰۰ کاربر/IP برای همان یک کانفیگ
- مقدار `0` برای مدت یعنی بی‌نهایت؛ ظرفیت پلن‌های فروش از ۱ تا ۱۰۰ کاربر/IP است
...
## تغییر v19.0.13 — تأیید خودکار درخواست مشتری ویژه

در پنل مدیر می‌توان تأیید خودکار **درخواست نمایندگی / ویژه‌شدن** را فعال کرد و زمان انتظار را به دقیقه تعیین کرد. فقط درخواست‌هایی که پس از این زمان در جدول درخواست نمایندگی همچنان `pending` باشند، خودکار تأیید و کاربر به مشتری ویژه تبدیل می‌شود. سفارش خرید و رسید پرداخت وارد این فرآیند نمی‌شوند.
