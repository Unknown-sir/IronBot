...
## تغییر v19.0.14 — ورود فقط هنگام نیاز

- بررسی IP/Session آیرون پنل با API Token انجام می‌شود و دیگر هر دقیقه درخواست `/login` ارسال نمی‌شود.
- ورود x-ui/3x-ui فقط هنگام عملیات واقعی موردنیاز انجام می‌شود و اطلاعات ورود خالی قبل از ارسال HTTP رد می‌شوند.
- Session پنل ریموت به‌صورت پیش‌فرض ۱۵ دقیقه Cache می‌شود تا لاگین‌های تکراری حذف شوند.
