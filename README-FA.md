...
## مسدود کردن / آنبن کاربران + مدیریت مدیرها + پلن یک‌کانفیگه


### مسدود کردن / آنبن کاربران

در این نسخه دکمه‌های **🚫 مسدود کردن کاربر** و **✅ آنبن کردن** به پنل مدیریت اضافه شده است:

1. مدیر روی **🚫 مسدود کردن کاربر** می‌زند و Chat ID کاربر مورد نظر را ارسال می‌کند.
2. ربات تأیید می‌گیرد و کاربر را مسدود می‌کند.
3. کاربر مسدود شده **هیچ پاسخی** از ربات دریافت نمی‌کند — پیام متنی، دکمه‌های شیشه‌ای و فایل‌های رسانه همگی نادیده گرفته می‌شوند.
4. با زدن **✅ آنبن کردن** فهرست کاربران مسدود شده نمایش داده می‌شود و با یک لمس می‌توان هر کاربر را آنبن کرد.
5. مدیرها قابل مسدود شدن نیستند.

اطلاعات بن در جدول `banned_users` (SQLite) ذخیره می‌شود و پس از ری‌استارت سرویس باقی می‌ماند.


### مدیریت مدیرها

در v19.1.0 بخش «👮 مدیریت مدیرها» به پنل ادمین اضافه شده است. مدیرها از داخل ربات فهرست می‌شوند و برای مدیرهای قابل‌حذف دکمه «🗑 حذف» با تأیید دو مرحله‌ای وجود دارد. مدیر اصلی (اولین Chat ID در `ADMIN_CHAT_IDS`)، آخرین مدیر و حساب مدیری که همان لحظه در حال مدیریت است قابل حذف نیستند. حذف مدیر فقط دسترسی مدیریتی او را برمی‌دارد و حساب تلگرام/کاربری عادی او را پاک نمی‌کند.
...
- پشتیبانی از چند مدیر ربات، افزودن و حذف امن مدیرها از پنل مدیریت
